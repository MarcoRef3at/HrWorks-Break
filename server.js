...
let runner = async (page, browser, action = 'start') => {
    try {
        if (action === 'stop') {
            var button = page.locator('text="Stop working hours recording "', { delay: 50 })
        } else if (action === 'start') {
            var button = page.locator('text="Start working hours recording "', { delay: 50 })
        }
...
async function hrWorksLogin() {
    const browser = await playwright.chromium.launch({ args: ['--start-maximized'], headless: true });
    ...
    await (await page.waitForSelector(userInput, { delay: 50 })).type(user, { delay: 50 });
    await (await page.waitForSelector(passwordInput, { delay: 50 })).type(password, { delay: 50 });
    await (await page.waitForSelector(companyInput, { delay: 50 })).type(company, { delay: 50 });
    ...
    isWorkingDay = await page.locator('text="Stop working hours recording "', { delay: 50 }).isVisible({ delay: 50 })
...
const breakRunner = async () => {
    ...
    if (isWorkingDay) {
        ...
        await sendEmail({email:"",subject:"Break Started",message:``})
        setTimeout(async () => {
            const { page, browser } = await hrWorksLogin();
            await runner(page, browser, 'start')
            await sendEmail({email:"",subject:"Break Ended",message:``})
        }, 30 * 60 * 1000);
    }
}
        ...
        try {
              if ((new Date()).getDay() != 6 || (new Date()).getDay() != 7) {
                  const { page, browser } = await hrWorksLogin();
                   await browser.close();
                if (!isWorkingDay) {
                    await sendEmail({email:"",subject:"Clock in reminder",message:`don't forget to clock-in \n ${url} `})
                }
        }
        } catch (error) {
...
var stopJob = new CronJob(
	...
	if ((new Date()).getDay() != 6 || (new Date()).getDay() != 7) {
	            ...
	            setTimeout(async () => {
	                ...
	                if (isWorkingDay) {
	                    await runner(page, browser, 'stop')
	                        await sendEmail({email:"",subject:"Clocked out",message:``})
	                }
	            }, rand * 60 * 1000)
	        }
} catch (error) {
